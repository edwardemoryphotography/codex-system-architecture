...
| TD-005 | Code Quality | Repeated `if (!supabase) return [...]` guard in 19 functions, each followed by a redundant re-check inside `client()`. Duplicated boilerplate; the dual guard can be consolidated. | `src/lib/supabase.ts` | S | Low | 2 | 1.0 | 2026-06-14 | Fixed in this PR |
